...
class User {
  login(req, res) {
    ...
    const strQry = `
        SELECT userID, firstName, lastName, gender, emaildAdd, userPass, userRole, userProfile FROM Users WHERE emailAdd = ${emailAdd};
        `; //verify email
      ...
      if (!data || data == null) {
        ...
        await compare(userPass, data[0].userPass, (cErro, cResult) => {
          ...
          if (cResult) {
            ...
            res.status(401).json({
              err: "Yoou entered an incorect password or nnot registered.",
            });
          }
        });
      }
  ...
  fetchUsers(req, res) {
    const strQry = `
        SELECT userID, firstName, lastName, gender, emaildAdd, userRole, userProfile FROM Users;
        `;
